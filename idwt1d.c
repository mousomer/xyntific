...
int main(int argc, char* argv[])
{
	...
	//int transformType = 2*(lifting>0) + (inPlace>0);
	...
	if ( dataType[0] == 'd' ) {
		...
		if ( lifting )
		{
			liftInvTransform_i(intInput, len, startHigh, inPlace, noRescale, wlp);
			if ( !noRescale )
				downScaleVect_i( intInput, len );
			if (verbose) displayArray_i(intInput, len, stdout );
			fwrite(intInput, nBytes, len, fpOut);
		}
		else
		{
			wavInvTransform_i(intInput, intOutput, len, startHigh, wfp);
			if ( !noRescale )
				downScaleVect_i( intInput, len );
			...
			if ( lifting )
			{
				liftInvTransform_f(floatInput, len, startHigh, inPlace, noRescale, wlp);
				if (verbose) displayArray_f(floatInput, len, stdout );
				fwrite(floatInput, nBytes, len, fpOut);
			}
			else
			{
				wavInvTransform_f(floatInput, floatOutput, len, startHigh, inPlace, wfp);
